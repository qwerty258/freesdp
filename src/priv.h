 ...
 * modifier if unknown.
 **/
 typedef struct _fsdp_bw_modifier_t
{
...
struct fsdp_media_description_t_s
{
    ...
    /* from i=<media title> */
    char *i_title;
    /* from `c=<network type> <address type> <connection address>' line (optional) */
    ...
    /* from `k=<method>' or `k=<method>:<encryption key>' line (optional) */
 ...
 **/
typedef struct _fsdp_time_period_t
{
    time_t start;
    time_t stop;
...
struct fsdp_description_t_s
{
    ...
    /* from `r=<repeat interval> <active duration> <list of offsets from start-time>' */
    fsdp_time_period_t **time_periods;
    unsigned int time_periods_count;
    /* from `z=<adjustment time> <offset> <adjustment time> <offset> ....' lines */
    char *timezone_adj;
    ...
    char **a_langs;
    unsigned int a_langs_count;
    /* from `m=<media> <port>/<number of ports> <transport> <fmt list>' lines [one or more] */
