...
#define TEMPINTS 2
    ...
    {
        p += 2;
        /* note that the following max lengths may vary in the future and are quite arbitary */
        if(sscanf(p, "%"MSFLENS"[\x21-\xFF] %"MSFLENS"[0-9] %"MSFLENS"[0-9] %2s %3s %"MSFLENS"s",
            fsdp_buf[0], fsdp_buf[1], fsdp_buf[2], fsdp_buf[3], fsdp_buf[4], fsdp_buf[5]) != 6)
        {
    ...
    if(!strncmp(p, "i=", 2) && sscanf(p, "i=%"MLFLENS"[^\r\n]", longfsdp_buf))
    {
    ...
    /* u=<URI> */
    if(!strncmp(p, "u=", 2) && sscanf(p, "u=%"MLFLENS"[^\r\n]", longfsdp_buf))
    {
    ...
    dsc->time_periods = calloc(dsc->time_periods_count, sizeof(fsdp_time_period_t*));
    index = 0;
                ...
                if(sscanf(p, "r=%10s %10s %"MLFLENS"[^\r\n]", fsdp_buf[0], fsdp_buf[1], longfsdp_buf) == 3)
                {
                    ...
                    if(result == FSDPE_OK)
                    {
                        ...
                        if(result == FSDPE_OK)
                        {
                            ...
                            i = longfsdp_buf;
                            for(k = 0; (k < repeat->offsets_count) && (result == FSDPE_OK); k++)
                            {
                                result = fsdp_repeat_time_to_uint(i, &(repeat->offsets[k]));
                                i = strchr(i, ' ');
                                if(NULL != i)
                                {
                                    i++;
    ...
    while(!strncmp(p, "a=", 2))
    {
        /* The "9" lenght specifier of the first string is subject to changes */
        if(sscanf(p, "a=%[^:\r\n]:%"MSFLENS"[^\r\n]", fsdp_buf[0], fsdp_buf[1]) == 2)
        {
            /* session-level value attributes */
            if(!strncmp(fsdp_buf[0], "cat", 3))
            {
                dsc->a_str_attributes[FSDP_SESSION_STR_ATT_CATEGORY] = strdup(fsdp_buf[1]);
            }
            else if(!strncmp(fsdp_buf[0], "keywds", 6))
            {
                dsc->a_str_attributes[FSDP_SESSION_STR_ATT_KEYWORDS] = strdup(fsdp_buf[1]);
            }
            else if(!strncmp(fsdp_buf[0], "tool", 4))
            {
                dsc->a_str_attributes[FSDP_SESSION_STR_ATT_TOOL] = strdup(fsdp_buf[1]);
            }
            ...
            {
                dsc->a_str_attributes[FSDP_SESSION_STR_ATT_CHARSET] = strdup(fsdp_buf[1]);
            }
            else if(!strncmp(fsdp_buf[0], "sdplang", 7))
            {
                if(NULL == dsc->a_sdplangs)
                {
                    ...
                    dsc->a_sdplangs = calloc(SDPLANGS_MAX_COUNT, sizeof(char*));
                }
                if(dsc->a_sdplangs_count < SDPLANGS_MAX_COUNT)
                {
                    dsc->a_sdplangs[dsc->a_sdplangs_count] = strdup(fsdp_buf[1]);
            ...
            else
            {
                ...
                *longfsdp_buf = '\0';
                ...
                if(NULL == dsc->unidentified_attributes)
                {
                    ...
                    dsc->unidentified_attributes = calloc(UNIDENTIFIED_ATTRIBUTES_MAX_COUNT, sizeof(char*));
                }
                if(dsc->unidentified_attributes_count < UNIDENTIFIED_ATTRIBUTES_MAX_COUNT)
                {
                    dsc->unidentified_attributes[dsc->unidentified_attributes_count] = strdup(longfsdp_buf);
    ...
    {  /* dsc->media_announcements_count > 0 */
        dsc->media_announcements = calloc(j, sizeof(fsdp_media_announcement_t*));
        for(j = 0; j < dsc->media_announcements_count; j++)
        {
            ...
            {
                dsc->media_announcements[j] = calloc(1, sizeof(fsdp_media_announcement_t));
                ...
                else if(!strncmp(fsdp_buf[0], "text", 4))
                {
                    media->media_type = FSDP_MEDIA_TEXT;
                }
            ...
            /* `c=' line (connection information - overrides session-level line) [optional if provided at session-level] */
            /* c=<network type> <address type> <connection address> */
            result = fsdp_parse_c(&p, &(media->c_network_type), &(media->c_address_type), &(media->c_address));
            if(result != FSDPE_OK)
            {
                return result;
            }
            ...
            while(!strncmp(p, "a=", 2))
            {
                if(sscanf(p, "a=%[^:\r\n]:%"MLFLENS"[^\r\n]", fsdp_buf[0], longfsdp_buf) == 2)
                {
                    ...
                    {
                        fsdp_parse_rtpmap(&(media->a_rtpmaps), &(media->a_rtpmaps_count), longfsdp_buf);
                    }
                    ...
                    else if(!strncmp(fsdp_buf[0], "sdplang", 7))
                    {
                        if(NULL == dsc->a_sdplangs)
                        {
                            ...
                            media->a_sdplangs = calloc(SDPLANGS_MAX_COUNT, sizeof(char*));
                        }
                        if(media->a_sdplangs_count < SDPLANGS_MAX_COUNT)
                        {
                            media->a_sdplangs[dsc->a_sdplangs_count] = strdup(longfsdp_buf);
                    ...
                    else if(!strncmp(fsdp_buf[0], "lang", 4))
                    {
                        if(NULL == dsc->a_langs)
                        {
                            ...
                            media->a_langs = calloc(SDPLANGS_MAX_COUNT, sizeof(char*));
                        }
                        if(media->a_langs_count < SDPLANGS_MAX_COUNT)
                        {
                            media->a_langs[dsc->a_langs_count] = strdup(longfsdp_buf);
                            media->a_langs_count++;
                        }
                    }
                    ...
                    else if(!strncmp(fsdp_buf[0], "fmtp", 4))
                    {
                        if(NULL == media->a_fmtps)
                        {
                            ...
                            media->a_fmtps = calloc(SDPLANGS_MAX_COUNT, sizeof(char*));
                        }
                        if(media->a_fmtps_count < SDPLANGS_MAX_COUNT)
                        {
                            media->a_fmtps[media->a_fmtps_count] = strdup(longfsdp_buf);
                            media->a_fmtps_count++;
                        }
                    }
                    else if(!strncmp(fsdp_buf[0], "rtcp", 4))
                    {
                        ...
                        opts = sscanf(longfsdp_buf, "%lu %2s %3s %"MSFLENS"s", &wuint[0], fsdp_buf[0], fsdp_buf[1], fsdp_buf[2]);
                        if(opts >= 1)
                        {
                            ...
                            if(opts >= 2)
                            {
                                ...
                                {
                                    media->a_rtcp_network_type = FSDP_NETWORK_TYPE_INET;
                                } /* else; TODO: define error code? */
                                if(opts >= 3)
                                {
                                    if(!strncmp(fsdp_buf[1], "IP4", 3))
                                    {
                                        media->a_rtcp_address_type = FSDP_ADDRESS_TYPE_IPV4;
                                    }
                                    else if(!strncmp(fsdp_buf[1], "IP6", 3))
                                    {
                                        media->a_rtcp_address_type = FSDP_ADDRESS_TYPE_IPV6;
                                    }
                                    else
                                    {
                                    ...
                                    if(opts >= 4)
                                    {
                                        media->a_rtcp_address = strdup(fsdp_buf[2]);
                                    }
                                }
                            }
                        }
                    ...
                    else
                    {
                        ...
                        *fsdp_buf[1] = '\0';
                        ...
                        if(NULL == media->unidentified_attributes)
                        {
                            ...
                            media->unidentified_attributes = calloc(UNIDENTIFIED_ATTRIBUTES_MAX_COUNT, sizeof(char*));
                        }
                        if(media->unidentified_attributes_count <
                         UNIDENTIFIED_ATTRIBUTES_MAX_COUNT)
                        {
                            media->unidentified_attributes[media->unidentified_attributes_count] = strdup(fsdp_buf[1]);
                    ...
                    else
                    {
                        ...
                        *longfsdp_buf = '\0';
                        ...
                        if(media->unidentified_attributes_count <
                         UNIDENTIFIED_ATTRIBUTES_MAX_COUNT)
                        {
                            media->unidentified_attributes[media->unidentified_attributes_count] = strdup(longfsdp_buf);
                            media->unidentified_attributes_count++;
                        }
                    }
                    NEXT_LINE(p);
    ...
    while(i > 0)
    {
        ...
        if(2 == sscanf(*p, "b=%"MSFLENS"[^:\r\n]:%lu", fsdp_buf, &wuint))
        {
            ...
            else
            {
                ...
                (*bw_modifiers)[index].b_unknown_bw_modt = (char *)strdup(fsdp_buf);
            }
...
{
    if((NULL == dsc) || (index >= dsc->emails_count))
      return NULL;
...
{
    if((NULL == dsc) || (index >= dsc->phones_count))
      return NULL;
...
const char *
fsdp_get_rtpmap_payload_type(const fsdp_description_t *dsc, unsigned int index)
{
    if((NULL == dsc) || (index >= dsc->a_rtpmaps_count))
      return NULL;
...
unsigned int
fsdp_get_rtpmap_clock_rate(const fsdp_description_t *dsc, unsigned int index)
{
    if((NULL == dsc) || (index >= dsc->a_rtpmaps_count))
      return 0;
...
fsdp_get_str_att(const fsdp_description_t *dsc, fsdp_session_str_att_t att)
{
    ...
    if(att <= FSDP_LAST_SESSION_STR_ATT)
      result = dsc->a_str_attributes[att];
    else
      result = NULL;
    return result;
}
...
{
    if((NULL == dsc) || (index >= dsc->a_sdplangs_count))
      return NULL;
...
{
    if((NULL == dsc) || (index >= dsc->a_langs_count))
      return NULL;
...
const char *
fsdp_get_media_format(const fsdp_media_description_t *dsc, unsigned int index)
{
    if((NULL == dsc) && (index < dsc->formats_count))
      return NULL;
...
fsdp_get_media_rtpmap_payload_type(const fsdp_media_description_t *mdsc,
                   unsigned int index)
{
    if((NULL == mdsc) || (index >= mdsc->a_rtpmaps_count))
      return NULL;
...
fsdp_get_media_sdplang(const fsdp_media_description_t *mdsc,
               unsigned int index)
{
    if((NULL == mdsc) || (index >= mdsc->a_sdplangs_count))
      return NULL;
...
fsdp_get_media_lang(const fsdp_media_description_t *mdsc,
               unsigned int index)
{
    if((NULL == mdsc) || (index >= mdsc->a_langs_count))
      return NULL;
...
const char *
fsdp_get_media_fmtp(const fsdp_media_description_t *mdsc, unsigned int index)
{
    if((NULL == mdsc) || (index >= mdsc->a_fmtps_count))
      return NULL;
...
fsdp_get_media_unidentified_attribute(const fsdp_media_description_t *mdsc,
                      unsigned int index)
{
    if(NULL == mdsc || (index < mdsc->unidentified_attributes_count))
      return NULL;
