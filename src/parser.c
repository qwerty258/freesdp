...
fsdp_parse(const char *text_description, fsdp_description_t *dsc)
{
	...
	if ( sscanf(p,"r=%10s %10s %"MLFLENS"[^\r\n]",
	  ...
	  if ( result == FSDPE_OK ) {
		...
		if ( NULL != i )
	      ...
	      i = longfsdp_buf;
	      for ( k = 0; 
		    (k < repeat->offsets_count) && (result == FSDPE_OK);
		    k++ ) {
  ...
  while ( !strncmp(p,"a=",2) ) {
	...
	dsc->a_str_attributes[FSDP_SESSION_STR_ATT_KEYWORDS] =
	  strdup(fsdp_buf[1]);
	...
	*longfsdp_buf = '\0';
	...
	if ( NULL == dsc->unidentified_attributes ) {
	  ...
	  dsc->unidentified_attributes = 
	    calloc(UNIDENTIFIED_ATTRIBUTES_MAX_COUNT,sizeof(char*));
	}
	if ( dsc->unidentified_attributes_count < 
	     UNIDENTIFIED_ATTRIBUTES_MAX_COUNT ) {
	  dsc->unidentified_attributes
	    [dsc->unidentified_attributes_count] = strdup(longfsdp_buf);
  ...
  if ( dsc->media_announcements_count == 0 ) {
	...
	if ( sscanf(p,"a=%[^:\r\n]:%"MLFLENS"[^\r\n]",fsdp_buf[0],longfsdp_buf) 
	  ...
	  if ( !strncmp(fsdp_buf[0],"ptime",5) )
	    ...
	    opts = sscanf(longfsdp_buf,"%lu %2s %3s %"MSFLENS"s",
	    ...
	    *fsdp_buf[1] = '\0';
	    ...
	    if ( media->unidentified_attributes_count < 
		 UNIDENTIFIED_ATTRIBUTES_MAX_COUNT ) {
	      media->unidentified_attributes
		[media->unidentified_attributes_count] = strdup(fsdp_buf[1]);
	  ...
	  if ( !strncmp(fsdp_buf[0],"recvonly",8) )
	    ...
	    *longfsdp_buf = '\0';
	    ...
	    if ( media->unidentified_attributes_count < 
		 UNIDENTIFIED_ATTRIBUTES_MAX_COUNT ) {
	      media->unidentified_attributes
		[media->unidentified_attributes_count] = strdup(longfsdp_buf);
	      media->unidentified_attributes_count++;
	    }
	  }
	  NEXT_LINE(p);
